
"use strict";
/* jshint
node : true
*/
// node rayTraceLauncher.js

const fs = require("fs");
const getLatestGameFile = file => fs.readFileSync(file).toString();
const getSingleFileGame = () => fs.readFileSync("rayTrace_Game.html").toString()
    .replace("INSERT_FRAG_SHADER_HERE", getLatestGameFile("rayTraceFragShader.c"))
    .replace("INSERT_VERT_SHADER_HERE", getLatestGameFile("rayTraceVertShader.c"))
    .replace("INSERT_GAME_CODE_HERE",   getLatestGameFile("rayTraceGameCode.js"))

const onGETrequest = function(request, response) {
    request.addListener("data", function(postDataChunk){console.log("Consuming data : " + postDataChunk);});
    request.addListener("end",  function(){
        response.writeHead(200);
        switch (request.url.toLowerCase()){
            case "/smashyball"  : response.end(getSingleFileGame());                    break;
            case "/favicon.ico" :
            case "/favicon.png" : response.end(fs.readFileSync("favicon.png"));         break;
            default             : response.end(fs.readFileSync(request.url));           break;
        }
        console.log(new Date() , request.url);
    });
};
const sslCert = {
    key  : "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----",
    cert : "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
};
const HTTPserver = require("https").createServer(sslCert, onGETrequest).listen(8000);

console.log("Listening on 8000...");